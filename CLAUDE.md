...
<rules>
- Never use cd command; run all commands directly using relative/absolute paths from the root directory
- No signatures: never add `Co-Authored-By: Claude <noreply@anthropic.com>` (or any variant) to commit messages
</rules>
...
- PRs will never include `CLAUDE.md` or fork-local `.gitignore` additions in their diff
</fork_specific>
